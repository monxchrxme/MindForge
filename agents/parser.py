...
class ParserAgent:
    ...
    def _decide_and_enrich(self, term: str, definition: str, context: str = "") -> str:
        ...
        decision_prompt = f"""Оцени определение концепта.
        ...
        ENRICH если:
            Только название без объяснения сути
            Определение-заглушка ("это механизм...", "процесс для...")
            Нет свойств или связей (примеры не нужны)
            Необязательно, чтобы определение было полностью раскрыто(можно опустить некоторые моменты, если есть определение и хотя бы 1 свойство) 

        KEEP если:
            Есть хотя бы 1 свойство  ИЛИ 1 связь с другими понятиями
            Указано практическое применение или назначение
            Определение дает базовое понимание концепта
            Необязательно, чтобы определение было полностью раскрыто(можно опустить некоторые моменты, если есть определение и хотя бы 1 свойство)   

        Примеры:

        1. "Класс — шаблон для создания объектов."
           → ENRICH (только название, нет деталей)

        2. "Класс — шаблон для создания объектов, содержащий атрибуты и методы."
           → KEEP (есть свойства в одном предложении)

        3. "Инкапсуляция — сокрытие данных."
           → ENRICH (нет объяснения КАК и ЗАЧЕМ)

        4. "Деструктор вызывается автоматически при удалении объекта для освобождения ресурсов."
           → KEEP (понятны суть и назначение)

        5. "Полиморфизм позволяет объектам разных классов использовать единый интерфейс через наследование."
           → KEEP (есть суть, механизм и связь)

        Ответ: ENRICH или KEEP?
        """.strip()

        try:
            # LLM принимает решение
            decision = self.client.generate(decision_prompt, ).strip().upper()
